...
<head>
<meta charset="UTF-8">
<title>main 컨텐츠</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/> <!-- 모바일 접속시 변경을 하기위한 장치 -->
...
<!-- bg-warning : 배경색을 지정한다. shadow(그림자효과추가) -->
<!-- mx-auto : 자동으로 크기를 적절하게 배치 -->
<!-- mt-5 : 박스가 브라우저 상단에 붙지 않도록 최대 마진을 설정 0-5까지 -->
<!-- p-5 : 박스 안쪽 여백을 설정 -->
<!-- w-75 : 박스의 크기를 브라우저 크기의 75%으로 유지 -->
	...
	<table align=center>
		<tr height=40>
			<td><b>방명록 리스트</b></td>
			<td align=right>
			<a href='feedadd2.jsp'><button type="button" class="btn btn-primary">글쓰기</button></a>
			</td>
		</tr>
		...
		String sql = "SELECT * FROM feed ORDER BY ts DESC "; //데이터 ts의 역순으로 가지고 오기
